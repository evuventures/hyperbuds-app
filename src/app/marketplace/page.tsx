...
// Removed 'Plus' as it was unused
import { Search, Store, Ticket, Globe, ArrowRight } from "lucide-react"; 
import Link from "next/link";

type TabType = "marketplace" | "bookings";
...
export const MarketplaceDashboard = () => {
  ...
  const { data: marketData, isLoading: marketLoading } = useQuery({
    ...
    queryFn: () => marketplaceApi.listServices({ 
      q: searchQuery, 
      category, 
      limit: 12 
    }),
  ...
  // Helper to extract the bookings array safely
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <header className="mb-12 space-y-8 flex flex-col items-center">
           ...
           <div className="text-center">
              <h1 className="text-5xl font-black tracking-tighter uppercase italic">Marketplace</h1>
              <p className="text-zinc-500 font-bold uppercase text-[10px] tracking-[0.2em] mt-2">
                {activeTab === 'marketplace' ? 'Browse Global Services' : 'Your Active Transactions'}
              </p>
           </div>

          {/* 3-TAB NAVIGATION - Updated class to rounded-3xl */}
          <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 p-2 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            <Link 
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all text-zinc-400 hover:text-zinc-800 dark:hover:text-white"
            >
        ...
        {activeTab === "marketplace" && (
          <div className="max-w-4xl mx-auto mb-12 space-y-6">
            <div className="relative group">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-pink-500 transition-colors" size={20} />
              <input 
                type="text" 
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar justify-center">
              {['Design', 'Development', 'Video', 'Audio', 'Writing'].map((cat) => (
                <button 
                  key={cat}
        ...
        {/* CONTENT AREA - Updated min-h class */}
        <main className="min-h-100">
          {activeTab === "marketplace" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {marketLoading ? (
                [...Array(6)].map((_, i) => (
                  <div key={i} className="h-80 bg-zinc-200 dark:bg-zinc-800 animate-pulse rounded-[2.5rem]" />
                ))
            ...
            <div className="max-w-3xl mx-auto space-y-4">
              {bookingsLoading ? (
                <div className="p-10 text-center animate-pulse uppercase font-black text-zinc-400">Loading your activity...</div>
              ) : bookings.length === 0 ? ( // Fixed length error
                <div className="p-20 text-center bg-white dark:bg-zinc-900 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-[3rem]">
                  ...
                  <p className="text-zinc-500 font-black uppercase tracking-widest text-sm">No active bookings found</p>
                </div>
              ) : (
                bookings.map((booking) => ( // Fixed map and any errors
                  <Link 
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black uppercase text-sm group-hover:text-pink-600 transition-colors">{booking.serviceId?.title || "Project Request"}</p>
                        <p className="text-[10px] text-zinc-500 font-bold uppercase">Status: <span className="text-pink-500">{booking.status}</span></p>
                      </div>
                    </div>
