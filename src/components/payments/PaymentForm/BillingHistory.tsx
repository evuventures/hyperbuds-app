...
export function BillingHistory({ className = '' }: BillingHistoryProps) {
    ...
    const loadPayments = async (page: number = 1, filters: Partial<PaymentHistoryQuery> = {}) => {
        try {
            ...
            // Construct URL search params from the filters and page
            ...
            // The paymentAPI.getPaymentHistory function likely expects a URL string,
            // not a query object, based on the error.
            const response = await paymentAPI.getPaymentHistory(`?${params.toString()}`);
    ...
    const handleFilterChange = (filter: 'status' | 'paymentType', value: string) => {
        const filters: Partial<PaymentHistoryQuery> = {};
        if (filter === 'status' && value !== 'all') {
            filters.status = value as 'succeeded' | 'failed' | 'pending' | 'canceled';
        }
        if (filter === 'paymentType' && value !== 'all') {
            filters.paymentType = value as 'subscription' | 'marketplace_service' | 'one_time';
        }
